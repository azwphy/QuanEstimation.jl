        ...
        for pk = 1:p_num
            ...
            for dk = 1:ctrl_num
                for ck = 1:ctrl_length
                    ...
                    +c2 *
                    rand(opt.rng) *
                    (gbest[dk, ck] - param_data(particles[pk]).ctrl[dk][ck])
    ...
    for ei = 1:(max_episode[1]-1)
        for pj = 1:p_num
            ...
            if f_now > p_fit[pj]
                ...
                p_out[pj] = f_out
                for di = 1:dim
                    pbest[di, pj] = state_data(particles[pj])[di]
                end
            end
        end
...
function optimize!(opt::Mopt_Projection, alg::PSO, obj, scheme, output)
    ...
    dim = get_dim(scheme)
    M_num = length(opt.M)
    particles = [[zeros(ComplexF64, dim) for j = 1:M_num] for i = 1:p_num]
    ...
    gbest = [zeros(ComplexF64, dim) for i = 1:M_num]
        ...
        for pk = 1:p_num
            ...
            for dk = 1:M_num
                for ck = 1:dim
                    ...
                    velocity[dk, ck, pk] =
                        c0 * velocity[dk, ck, pk] +
                        c1 * rand(opt.rng) * (pbest[dk, ck, pk] - particles[pk][dk][ck])
...
function optimize!(opt::Mopt_LinearComb, alg::PSO, obj, scheme, output)
    ...
    ini_particle = ini_particle[1]
    particles = [[zeros(basis_num) for j = 1:M_num] for i = 1:p_num]
    ...
    M = [sum([particles[1][i][j] * POVM_basis[j] for j = 1:basis_num]) for i = 1:M_num]
    obj_copy = set_M(obj, M)
    f_ini, f_comp = objective(obj_copy, scheme)
    ...
    for ei = 1:(max_episode[1]-1)
        for pj = 1:p_num
            M = [
                sum([particles[pj][i][j] * POVM_basis[j] for j = 1:basis_num]) for
                i = 1:M_num
            ]
            obj_copy = set_M(obj, M)
            f_out, f_now = objective(obj_copy, scheme)
...
function optimize!(opt::Mopt_Rotation, alg::PSO, obj, scheme, output)
    ...
    # initialization  
    particles = [zeros(dim^2) for i = 1:p_num]
    ...
    for ei = 1:(max_episode[1]-1)
        ...
        for pk = 1:p_num
            ...
            for ck = 1:(dim^2)
                ...
                velocity[ck, pk] =
                    c0 * velocity[ck, pk] +
                    c1 * rand(opt.rng) * (pbest[ck, pk] - particles[pk][ck]) +
                    c2 * rand(opt.rng) * (gbest[ck] - particles[pk][ck])
            ...
            for dk = 1:ctrl_num
                for ck = 1:ctrl_length
                    ...
                    velocity_ctrl[dk, ck, pk] =
                        ...
                        c1 *
                        rand(opt.rng) *
                        (pbest_ctrl[dk, ck, pk] - param_data(particles[pk]).ctrl[dk][ck])
                    +c2 *
                    rand(opt.rng) *
                    (gbest_ctrl[dk, ck] - param_data(particles[pk]).ctrl[dk][ck])
...
function optimize!(opt::StateMeasurementOpt, alg::PSO, obj, scheme, output)
    ...
    gbest_meas = [zeros(ComplexF64, dim) for i = 1:M_num]
    ...
    # initialization  
    initial_state!(psi0, particles, p_num, opt.rng)
    C_all = [[zeros(ComplexF64, dim) for j = 1:M_num] for i = 1:p_num]
            ...
            for dk = 1:M_num
                for ck = 1:dim
                    ...
                    velocity_meas[dk, ck, pk] =
                        ...
                        c1 * rand(opt.rng) * (pbest_meas[dk, ck, pk] - C_all[pk][dk][ck])
...
function optimize!(opt::ControlMeasurementOpt, alg::PSO, obj, scheme, output)
    ...
    set_f!(output, f_ini)
    set_buffer!(output, param_data(particles[1]).ctrl, M)
    set_io!(output, f_ini)
    show(opt, output, obj, alg)
    ...
    for ei = 1:(max_episode[1]-1)
        for pj = 1:p_num
            ...
            if f_now > p_fit[pj]
                ...
                p_out[pj] = f_out
                for di = 1:dim
                    pbest_state[di, pj] = particles[pj].StatePreparation.data[di]
                end
                for di = 1:ctrl_num
                    for ni = 1:ctrl_length
                        pbest_ctrl[di, ni, pj] = param_data(particles[pj]).ctrl[di][ni]
