...
function optimize!(opt::Mopt_Projection, alg::DE, obj, scheme, output)
    ...
    populations = [[zeros(ComplexF64, dim) for j = 1:M_num] for i = 1:p_num]
    # initialization  
...
function optimize!(opt::Mopt_LinearComb, alg::DE, obj, scheme, output)
    ...
    populations = [[zeros(basis_num) for j = 1:M_num] for i = 1:p_num]
    ...
    M = [sum([populations[1][i][j] * POVM_basis[j] for j = 1:basis_num]) for i = 1:M_num]
    set_f!(output, p_out[1])
    set_buffer!(output, M)
    ...
    for ei = 1:(max_episode-1)
        ...
        idx = findmax(p_fit)[2]
        M = [
            sum([populations[idx][i][j] * POVM_basis[j] for j = 1:basis_num]) for
            i = 1:M_num
        ]
        set_f!(output, p_out[idx])
        set_buffer!(output, M)
...
function optimize!(opt::StateControlOpt, alg::DE, obj, scheme, output)
    ...
    for ei = 1:(max_episode-1)
        for pj = 1:p_num
            ...
            for ci = 1:dim
                state_mut[ci] =
                    state_data(populations[mut_num[1]])[ci] +
                    c * (
                        state_data(populations[mut_num[2]])[ci] -
                        state_data(populations[mut_num[3]])[ci]
                    )
            end
            ctrl_mut = [Vector{Float64}(undef, ctrl_length) for i = 1:ctrl_num]
            for ci = 1:ctrl_num
            ...
            if f_cross > p_fit[pj]
                ...
                for ck = 1:ctrl_num
                    for tk = 1:ctrl_length
                        populations[pj].Parameterization.data.ctrl[ck][tk] =
                            ctrl_cross[ck][tk]
                    end
                end
            end
...
function optimize!(opt::StateControlMeasurementOpt, alg::DE, obj, scheme, output)
    ...
    for ei = 1:(max_episode-1)
        ...
        set_buffer!(
            output,
            state_data(populations[idx]),
            param_data(populations[idx]).ctrl,
            M,
        )
        set_io!(output, p_out[idx], ei)
        show(output, obj)
