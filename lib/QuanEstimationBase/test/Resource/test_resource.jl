...
using QuanEstimationBase: SpinSqueezing, TargetTime
...
function test_TargetTime()
    ...
    @test TargetTime(0.0, tspan, testfunc, omega) ≈ pi / 2 atol = 1e-6

    # Test print output when target is not found
    @test TargetTime(2.0, tspan, testfunc, omega) == nothing
end

@testset "SpinSqueezing" begin
    test_resource()
end
@testset "TargetTime" begin
    test_TargetTime()
end
